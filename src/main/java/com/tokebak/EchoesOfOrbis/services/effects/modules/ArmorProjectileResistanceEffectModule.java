...
import com.tokebak.EchoesOfOrbis.services.effects.processors.ArmorProjectileResistanceProcessor;
import com.tokebak.EchoesOfOrbis.services.effects.processors.EffectProcessor;

import javax.annotation.Nonnull;

/**
 * ARMOR_PROJECTILE_RESISTANCE: projectile damage reduction. Applied as stat modifier from armor.
 */
public class ArmorProjectileResistanceEffectModule implements EffectModule {
    ...
    private static final String SHORT_DESCRIPTION = "Projectile resistance";

    private final WeaponEffectDefinition definition = WeaponEffectDefinition.builder(WeaponEffectType.ARMOR_PROJECTILE_RESISTANCE)
            .baseValue(BASE_VALUE)
            .valuePerLevel(VALUE_PER_LEVEL)
            .maxValue(MAX_VALUE)
            .maxLevel(MAX_LEVEL)
            .description("+{value} projectile resistance")
            .valueDisplayFormat(ValueDisplayFormat.PERCENT)
            .build();

    private final EffectProcessor processor = new ArmorProjectileResistanceProcessor();

    @Override
    @Nonnull
    public WeaponEffectType getType() {
        return WeaponEffectType.ARMOR_PROJECTILE_RESISTANCE;
    }
