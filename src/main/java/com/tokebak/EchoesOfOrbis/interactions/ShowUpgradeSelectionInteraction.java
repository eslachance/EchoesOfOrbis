...
public final class ShowUpgradeSelectionInteraction extends SimpleInstantInteraction {
    ...
    protected void firstRun(
        ...
        EooLogger.debug("EOO_ShowUpgradeSelection firstRun invoked, type=%s, targetBlock=%s, targetEntity=%s",
                type, context.getTargetBlock(), context.getTargetEntity());

        if (context.getTargetEntity() != null) {
            EooLogger.debug("Interaction FAIL: targetEntity exists, UseEntity should have handled this");
            ...
            if (chunkReference != null && chunkReference.isValid()) {
                ...
                if (worldChunk != null) {
                    ...
                    if (BlockHarvestUtils.shouldPickupByInteraction(blockType)) {
                        EooLogger.debug("Interaction FAIL: targetBlock is harvestable (%s), letting BreakBlock handle it",
        ...
        if (itemExpService == null) {
            EooLogger.warn("Interaction FAIL: itemExpService=null");
            context.getState().state = InteractionState.Failed;
            return;
        }
        ...
        if (playerRef == null || !playerRef.isValid()) {
            EooLogger.debug("Interaction FAIL: playerRef=null or invalid");
            context.getState().state = InteractionState.Failed;
            return;
        }
        ...
        if (playerRefComponent == null) {
            EooLogger.debug("Interaction FAIL: playerRefComponent=null");
            context.getState().state = InteractionState.Failed;
            return;
        }
        ...
        if (player == null) {
            EooLogger.debug("Interaction FAIL: player=null");
            context.getState().state = InteractionState.Failed;
            return;
        }
        ...
        if (baubleContainerService == null) {
            EooLogger.warn("Interaction FAIL: baubleContainerService=null");
            context.getState().state = InteractionState.Failed;
            return;
        }
        ...
        EooLogger.debug("Interaction SUCCESS: opened main EOO UI");
